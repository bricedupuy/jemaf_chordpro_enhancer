...
This script processes ChordPro files to:
1. Enhance them with metadata from CSV
2. Deduplicate repeated sections
3. Generate FreeShow .show files
4. Handle French punctuation with proper non-breaking spaces

Usage: python chordpro_processor.py [input_dir] [csv_file] [output_dir]
"""
            ...
            with open(csv_file, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    metadata[key] = SongMetadata(
                        ...
                        key=clean_row.get('Tonalit̩', ''),  # Note: might need encoding fix
                        format=clean_row.get('Format', ''),
                        copyright=clean_row.get('Copyright', ''),
                        reference=clean_row.get('R̩f̩rence', ''),  # Note: might need encoding fix
                        theme=clean_row.get('Th̬me', ''),  # Note: might need encoding fix
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: python chordpro_processor.py <input_dir> <csv_file> <output_dir>")
        print("  input_dir: Directory containing .chordpro files")
        print("  csv_file: CSV file with song metadata")  
        print("  output_dir: Directory to save enhanced files and .show files")
        sys.exit(1)
    ...
    for filename in chordpro_files:
        ...
        try:
            ...
            enhanced_path = processor.enhance_chordpro(filepath, output_dir)
            print(f"  Enhanced: {os.path.basename(enhanced_path)}")
            
            # Generate FreeShow file
            show_path = processor.generate_freeshow_file(enhanced_path, output_dir)
